...
void display() {
    ...
    // Draw the blue stripe of the French flag
    glColor3f(0.0, 0.0, 1.0); // Blue
    glBegin(GL_POLYGON);
    glVertex2f(xOffset, yOffset);
    glVertex2f(xOffset + flagWidth / 3, yOffset);
    glVertex2f(xOffset + flagWidth / 3, yOffset + flagHeight);
    glVertex2f(xOffset, yOffset + flagHeight);
    glEnd();

    // Draw the white stripe of the French flag
    glColor3f(1.0, 1.0, 1.0); // White
    glBegin(GL_POLYGON);
    glVertex2f(xOffset + flagWidth / 3, yOffset);
    glVertex2f(xOffset + 2 * flagWidth / 3, yOffset);
    glVertex2f(xOffset + 2 * flagWidth / 3, yOffset + flagHeight);
    glVertex2f(xOffset + flagWidth / 3, yOffset + flagHeight);
    glEnd();

    // Draw the red stripe of the French flag
